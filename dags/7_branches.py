...
def branches_dag():
    ...
    def extract_data(**kwargs):
        ...
        is_weekend = True  # Simulating a condition for branching
        extracted_data = {
    ...
    def decide_branch(**kwargs):
        ti = kwargs['ti']
        is_weekend = ti.xcom_pull(task_ids='extract_data', key='extracted_data')['is_weekend']
        if is_weekend:
            return 'no_load_data'
        else:
            return 'load_data'
    ...
    def no_load_data(**kwargs):
        print("This task does not load data and is used to demonstrate branching.")
    ...
    no_load_data_task = no_load_data()
    decide_branch_task = decide_branch()
    
    extract_task >> [transform_api_task, transform_db_task, transform_file_task] >> decide_branch_task >> [load_data_task, no_load_data_task]
